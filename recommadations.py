...
def calculate_total_score(skill_match, experience_match, location_match, pr_points_match, difficulty_level,
                          success_rate, cost, duration):
    ...
    # Calculate the total score with normalized values
    total_score = (
            (skill_match * 0.35) +  # Higher weight for skills
            ...
            (success_rate * 0.15) +  # Success rate holds a high weight
            ((100 - difficulty_level) * 0.1) +  # Lower difficulty will slightly increase the score
            ((100 - normalized_cost) * 0.025) +  # Lower normalized cost increases the score slightly
            ((100 - normalized_duration) * 0.025)  # Shorter normalized duration increases the score slightly
    )
...
def rank_and_categorize_pathways(user_data, pathways):
    ...
    for pathway in pathways:
        ...
        if total_score >= 80:
            recommendations['fully_qualified'].append((pathway, total_score))
        elif total_score >= 40:
            recommendations['partially_qualified'].append((pathway, total_score))
        else:
            recommendations['potential_interest'].append((pathway, total_score))
...
def recommend_pr_pathways(user, db):
    ...
    # Display or return recommendations (You can replace this with your own logic)
    for category, paths in recommendations.items():
        print(f"\n{category.upper()} PATHWAYS:")
        for pathway, score in sorted(paths, key=lambda x: x[1], reverse=True):
            print(f"{pathway['pathway_name']} - Score: {score:.2f}")
...
recommend_pr_pathways({"_id": ObjectId(user_id)}, db)
